...
- `mnemos serve [--grpc-port N]` — REST + gRPC API service (the "as a service" surface; JWT auth, TLS/mTLS, multi-tenant namespace/RLS). `serve --require-tenant` (ADR 0007) makes every REST **and** gRPC request scope to its token's `tnt` claim via a per-request tenant connection (Postgres RLS or namespace-per-tenant for sqlite/mysql/local libSQL — see the MCP note above): REST auths reads too and a `tenantScopeMiddleware` opens the tenant conn (handlers resolve it through `scopedConn/scopedWriter/scopedMem` in `serve_tenant.go`); gRPC's interceptor opens it and methods resolve via `connFor/writerFor/memFor` (`internal/server/grpc`). Infra endpoints (`/health`, `/`, `/app`, `/internal/metrics`) stay unauthenticated. Guarded by `TestPostgres_CrossTenantIsolation`, `TestGRPC_CrossTenantIsolation`, and `TestGRPC_CrossTenantIsolation_SQLite`. Distinct from the MCP surface; API parity across MCP/HTTP/gRPC is enforced by `TestAPISurfaceParity`.
