...
// TestMySQL_ClaimConfidenceComponentsRoundTrip verifies the ADR-0014/0013 credit +
// salience audit map (confidence_components) persists and round-trips on MySQL, and
// that the ports.BeliefCreditWriter capability writes it in place.
func TestMySQL_ClaimConfidenceComponentsRoundTrip(t *testing.T) {
	conn := withConn(t)
	ctx := context.Background()
	now := time.Now().UTC().Truncate(time.Microsecond)
	...
	if !ok {
		t.Fatal("mysql ClaimRepository does not implement ports.BeliefCreditWriter")
	}
...
// TestMySQL_RelationshipStrengthRoundTrip verifies the ADR-0015 §4 Hebbian
// strength column persists on MySQL: new edges default to 1.0, ListByClaimIDs
...
func TestMySQL_RelationshipStrengthRoundTrip(t *testing.T) {
	conn := withConn(t)
	ctx := context.Background()
	now := time.Now().UTC()
	...
	if !ok {
		t.Fatal("mysql RelationshipRepository does not implement ports.RelationshipStrengthener")
	}
...
// TestMySQL_RelationshipDecay verifies the ADR-0015 §5 decay on MySQL: an
// over-base edge is pulled toward the base 1.0, a base edge is untouched.
func TestMySQL_RelationshipDecay(t *testing.T) {
	conn := withConn(t)
	ctx := context.Background()
	now := time.Now().UTC()
	if err := conn.Claims.Upsert(ctx, []domain.Claim{
