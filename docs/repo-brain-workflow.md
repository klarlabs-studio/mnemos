...
`CLAUDE.md`, or `--file <name>` for a custom filename. Because Claude Code and
other agents auto-load `AGENTS.md` / `CLAUDE.md`, these learnings are followed
natively.

**Refreshing the doc.** Run `mnemos sync-docs` when you want to publish the
brain's latest learnings into the committed doc — it is a deliberate,
non-destructive step (it absorbs any edits you made inside the block first, then
regenerates). A session `capture` **absorbs** edits into the brain but does
**not** rewrite the doc, so it can never clobber a hand-written note. A tidy way
to keep the committed doc current is a git pre-commit hook:

```bash
# .git/hooks/pre-commit
mnemos sync-docs && git add AGENTS.md
```
...
```
   ...
   └───────┬──────────────────────────────────▲─────────────┘
           │                                    │
   sync-docs (explicit refresh)       brief + capture sync-back
...
1. **Brain → AGENTS.md.** `mnemos sync-docs` writes decisions, top-trust facts,
   and open questions into the managed block (a deliberate refresh).
2. **Human edits.** You refine wording or add a line inside the markers and
   commit the file.
3. **AGENTS.md → brain.** `brief` (session start) and `capture` (session end)
   detect edits **inside** the managed block by comparing against a content hash
   in `.mnemos/.<file>.sha`, and ingest the changed lines into the repo brain.
   The pipeline's dedup collapses lines the brain already knows. (Brief runs a
   fast rule-based pass so session start never stalls; capture does the thorough
   LLM pass.)

**Safety:** sync-back never rewrites the doc, so a note that didn't extract into
a claim is never wiped. The managed block is regenerated only by an explicit
`mnemos sync-docs`, which absorbs your edits first.
