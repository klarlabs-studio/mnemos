...
// The harness wraps local `!`-command output in <local-command-caveat> /
// <local-command-stdout> blocks. The bare "Caveat:" line was already dropped,
// but the wrapper and the command stdout inside it reached extraction and
// showed up as stored claims in a real brain.
func TestStripHarnessText_LocalCommandBlocks(t *testing.T) {
	tests := []struct {
		name string
		in   string
		want string
		...
		{
			name: "caveat + stdout wrapper removed",
			in:   "Real question here.\n<local-command-caveat>Caveat: The messages below were generated by the user while running local commands.</local-command-caveat>",
			want: "Real question here.",
		},
		{
			name: "stdout block removed",
			in:   "<local-command-stdout>total 42\ndrwxr-xr-x  5 user  staff</local-command-stdout>",
			want: "",
		},
		{
			name: "unclosed caveat line dropped",
			in:   "<local-command-caveat>Caveat: the messages below\nActual content survives.",
			want: "Actual content survives.",
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := stripHarnessText(tt.in); got != tt.want {
				t.Errorf("stripHarnessText()\n got: %q\nwant: %q", got, tt.want)
			}
		})
	}
...
// A sentence that merely mentions a command must survive.
func TestStripHarnessText_KeepsCommandMentions(t *testing.T) {
	keep := "the local command runner shells out to /bin/sh"
	if got := stripHarnessText(keep); got != keep {
		t.Errorf("dropped a real sentence about commands: %q", got)
	}
}
