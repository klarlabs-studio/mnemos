...
func TestServe_ExpectationLifecycle(t *testing.T) {
	...
	resp := postJSON(t, srv.URL+"/v1/beliefs/cl_pred/expectation", map[string]any{
	...
	resp2 := postJSON(t, srv.URL+"/v1/beliefs/cl_pred/observation", map[string]any{"observed": 104.0}, authHdr)
	...
	// 3) Read it back.
	got := getJSON(t, srv.URL+"/v1/beliefs/cl_pred/expectation", authHdr)
...
func TestServe_ExpectationNotFound(t *testing.T) {
	...
	srv := httptest.NewServer(newServerMux(conn))
	defer srv.Close()
	resp, err := http.Get(srv.URL + "/v1/beliefs/cl_missing/expectation")
	if err != nil {
		t.Fatalf("get: %v", err)
	}
